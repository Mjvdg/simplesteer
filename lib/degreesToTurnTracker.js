...
const DegreesToTurnTracker = (function () {
  ...
  function updateReverse(currentLocation){
    const reverseDriveBearing = State.driveBearing - 180;
    const antennaToRearAxleDistance = 1.2; //in meter
    const rearAxlePosition =  turf.rhumbDestination(currentLocation, antennaToRearAxleDistance / 1000, reverseDriveBearing).geometry.coordinates;
    //target line
    const state = State;
    ...
    var tractor    = new Tractor(currentLocation, -State.driveBearing, state);
   ...
   // let toFar     = targetLine.tractorToFar();
    ...
    var turnAngle = targetLine.tractorTurnDirection(targetLine.toClose, targetLine.toFar)*targetLine.tractor.maxSteeringAngle;
    // Correction for distance to line and speed.
    // Correction for distance to line and speed.
    var scaledTurnAngle = turnAngle*(targetLine.alignmentFactor()*targetLine.speedFactor());
    // console.log('toFar', toFar);
    // console.log('toClose', toClose)
     console.log('scaledTurnAngle', scaledTurnAngle)
    State.degreesToTurn = scaledTurnAngle
  }
