...
class RtkClient{
  ...
  _processData(input){
    ...
    // State.eventEmitters.rtkclient.emit(`receiver${this.nr}`, receiver);
    GpsFuser.fuseReceiver(this.nr, receiver)
    sendInfo(this.nr, receiver);
    // ioRoot.emit(`info${nr}`, `${time} </br>Sats: ${satCount} </br>${strQuality}`);
    // ioSettings.emit(`statusRTKLIB${nr}`, strQuality);

    
    // if(nr === 1){
    //     GPSFUSER.addData1(Latitude, Longitude, height, dateTime);
    //     ioRoot.emit('isABset', { pointA: (pointA !== undefined), pointB: (pointB !== undefined) });
    // }else if(nr === 2){
    //     GPSFUSER.addData2(Latitude, Longitude, height, dateTime);
    // }
  }
