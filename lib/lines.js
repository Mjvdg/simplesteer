...
const Lines = (() => {
    const createAndSend = () => {
        ...
        function createLine(offset){
            const offsetPointA = Calc.coord(pointA, abLineBearing - 90, (-closestLineNr + offset) * DB.getSettings().machineWidth /100);
            const offsetPointB = Calc.coord(pointB, abLineBearing - 90, (-closestLineNr + offset) * DB.getSettings().machineWidth /100);
