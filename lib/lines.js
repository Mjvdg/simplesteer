...
const getClosestCurveLineNr = require('./getClosestCurveLineNr');
...
const Lines = (() => {
    ...
    function createAndSendCurved() {
        ...
        const closestLineNr = getClosestCurveLineNr([currentLocation.longitude, currentLocation.latitude]);
        ...
        function createLine(offset) {
            let line = turf.lineOffset(turf.lineString(State.curvedLines.abLine), (((-closestLineNr + offset) * DB.getSettings().machineWidth / 100) + lineOffset) / 1000).geometry.coordinates;
            return line;
        }
    ...
    function createAndSendStraight() {
        ...
        function createLine(offset) {
            ...
            return [
                [offsetPointA.longitude, offsetPointA.latitude],
                [offsetPointB.longitude, offsetPointB.latitude]
            ];
        }
        IoSend.sendMapLines();
    }
