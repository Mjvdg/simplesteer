...
io.on('connection', (socket) => {
  ...
  socket.on('saveTargetPointSettings', ({ timeAheadSeconds, minimumDistance }) => {
    ...
    IoSend.sendTargetPointSettingsSaveSuccess(socket);
  });
  socket.on('getTargetPointSettings', () => {
    IoSend.sendTargetPointSettings(socket);
  });
  socket.on('saveSteeringSettings', ({ method, mode, pid, toleranceDegrees }) => {

    DB.setSteeringSettings(method, mode, pid, toleranceDegrees);
  });
