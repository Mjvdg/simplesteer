...
function SendNewUserData(socket) {
  ...
  sendIsCurvedRecording(socket);
  sendIsWorkRecording(socket);
...
function sendIsCurvedRecording(socket) {
  if (!socket) {
    socket = io;
  }
  socket.emit('isCurvedRecording', State.curvedLines.isRecording);
}

function sendIsWorkRecording(socket){
  if (!socket) {
    socket = io;
  }
  socket.emit('isWorkRecording', State.isWorkRecording);
}
...
function sendFastRecordedPoints(points){
  io.emit('fastRecordedPoints', points);
}
function sendBigRecordedWorkPoints(socket){
  if (!socket) {
    socket = io;
  }
  socket.emit('bigRecordedWorkPoints', State.bigWorkPolygons)
}

function sendRecordedArea(socket){
  if (!socket) {
    socket = io;
  }

  socket.emit('recordedArea', State.recordedArea);
}
module.exports = {
  init,
  sendInfo,
  ...
  sendFields,
  sendFastRecordedPoints,
  sendBigRecordedWorkPoints,
  sendRecordedArea,
};