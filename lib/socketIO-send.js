...
function sendWheelAndTargetPointLocation(socket) {
  ...
  socket.emit('targetPointLocation',
    {
      targetPointLocation: State.targetPointLocation,
...
function sendSteeringSettings(socket) {
  ...
  let { method, mode, pid, toleranceDegrees, minimumMotorPwm } = DB.getSteeringSettings();
  socket.emit('steeringSettings', { method, mode, pid, toleranceDegrees, minimumMotorPwm })
}
...
function sendCurvedLine() {
  io.emit('curvedAbLine', State.curvedLines.curvedAbLine);
}
