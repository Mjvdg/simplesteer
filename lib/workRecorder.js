...
const WorkRecorder = (() => {
  ...
  function setIsRecording(isRec) {
    ...
    if (!isRec) {
      let bigLastIndex = bigWorkPolygons.length - 1;
     bigWorkPolygons[bigLastIndex].push(bigWorkPolygons[bigLastIndex][0]); //close polygon
      bigWorkPolygons.push([]);
  ...
  function updateLocation(location) {
    ...
    if (isRecording) {
      bigWorkPolygons[bigWorkPolygons.length - 1].unshift(leftpoint);
      bigWorkPolygons[bigWorkPolygons.length - 1].push(rightPoint);
    ...
    if (fastCouter > 25) {
      fastCouter = 0;
      IoSend.sendBigRecordedWorkPoints(bigWorkPolygons);
    }
