...
function saveStraightABlines(name) {
  db.get('fields')
    .push({ id: Date.now(), name, type: 'straightABlines', recordedWork: State.bigWorkPolygons, pointA: State.pointA.location, pointB: State.pointB.location})
    .write();
}
function saveCurvedABlines(name) {
  db.get('fields')
    .push({ id: Date.now(), name, type: 'curvedABlines', recordedWork: State.bigWorkPolygons, abLine: State.curvedLines.abLine })
    .write();
}
