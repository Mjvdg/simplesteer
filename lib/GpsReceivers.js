...
if (Config.simulator) {
  ...
  const childLeft = fork(program, ['../ubxSimulator/logsLeft2.ubx'], {stdio: [ 'pipe', 'pipe', 'pipe', 'ipc' ]});
  childLeft.on('message', message => {
    message.data = [181, 98,...message.data]
    processUbxLeft(Buffer.from(message.data));
  });
  const childRight = fork(program, ['../ubxSimulator/logsRight2.ubx'], {stdio: [ 'pipe', 'pipe', 'pipe', 'ipc' ]});
