...
if (Config.simulator) {
  ...
  const childLeft = fork(program, ['../ubxSimulator/logsLeftFix.ubx'], {stdio: [ 'pipe', 'pipe', 'pipe', 'ipc' ]});
  childLeft.on('message', message => {
    message.data = [181, 98,...message.data]
    processUbxLeft(Buffer.from(message.data));
  });
  const childRight = fork(program, ['../ubxSimulator/logsRightFix.ubx'], {stdio: [ 'pipe', 'pipe', 'pipe', 'ipc' ]});

  childRight.on('message', message => {
    message.data = [181, 98,...message.data]
    processUbxRight(Buffer.from(message.data));
  });
